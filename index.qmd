...
eoh-derived wall-window PID as the always-on safety floor, plus a health-gated quadratic wall fit that adds heading feedforward and a curvature-based speed cap — *only when trusted*.
:::
...
<svg viewBox="0 0 960 380" xmlns="http://www.w3.org/2000/svg" width="100%"
  ...
  <text x="286" y="35" text-anchor="middle" font-family="monospace" font-size="9" fill="#00d4ff" letter-spacing="2">EOH INNER LOOP</text>
  ...
  <text x="286" y="254" text-anchor="middle" font-family="sans-serif" font-size="11" font-weight="600" fill="#00d4ff">steer_eoh, v_eoh</text>
  ...
  <text x="673" y="124" text-anchor="middle" font-family="sans-serif" font-size="11" font-weight="600" fill="#ffd166">clip(steer_eoh + ff)</text>
  ...
  <text x="673" y="224" text-anchor="middle" font-family="sans-serif" font-size="11" font-weight="600" fill="#ffd166">min(v_eoh, v_curv)</text>
...
*System architecture. The LiDAR scan feeds both the eoh PID inner loop (always on) and the wide-sector wall fits (gated by a multi-frame health check). The combiner clips the heading feedforward inside ±0.12 rad and applies the curvature speed cap only downward — the inner loop remains the safety floor.*
...
A prior team member's real-vehicle implementation (the *eoh* stack — a ±60° wall-window PID on the LiDAR range difference, with speed proportional to forward clearance) is the starting point. The question we investigate is:

> *Can we add fit-based features (quadratic wall curvature, heading estimate) on top of eoh to improve centerline tracking, without ever reducing its robustness floor?*
...
::: {.video-container}
**LiDAR ROSBAG #1 — annotated.** Offline replay of a real-car LiDAR rosbag, processed by the same perception path used live. The ±60° eoh windows and the wide fit sectors are both drawn.
...
### 1. eoh inner loop (safety floor)

A direct port of the real-car implementation. Two 11-ray windows are taken at ±60°; the **steering error** is the mean-range difference `d_L − d_R` fed through a PID with `Kp=0.6, Ki=0.005, Kd=0.2` and an integral clamp of 20. A ±10° forward cone supplies the minimum clearance used as **speed command** `v = clip(d_front · 2, 0, v_max)`.
...
This loop is *always on*. Setting `enable_advanced: false` reverts to byte-identical eoh behavior — the augmentation can never take the controller below this floor.
...
When both sides are healthy, a **heading feedforward** `ff = k · ½(b_L + b_R)` is clipped to ±0.12 rad (~30% of max steer) and added to the PID's output. A **curvature-based speed cap** `v_curv = √(a_lat / |κ|)` is derived from the fit's quadratic coefficients and applied as `min(v_eoh, v_curv)` — it can only *lower* the eoh speed, never raise it.

### 3. Bounded combiner

```python
steer_cmd = clip(steer_eoh + ff,              -0.4, 0.4)  # inner-loop clip
speed_cmd = min(speed_eoh, v_curv)                        # cap-only
```
...
If any of those fail, `ff = 0` and `v_curv = ∞` on that frame — the advanced layer silently steps aside, the eoh PID drives the car by itself.

The consequence is a strictly-bounded perturbation: the worst case of the advanced layer is "same as eoh." In 2×60 s of sim driving across all our tracks, zero collisions occurred in either condition.
...
## Results — eoh vs Advanced {#results .section-title}
...
| metric | eoh (m) | advanced (m) | Δ (adv − eoh) | % change |
|--------|---------|--------------|---------------|----------|
...
|-----------|-------|-------|-------|------------------------|-------|
| eoh       | 14.95 s | 15.00 s | 15.01 s | **15.00 ± 0.03 s** | 1.513 m/s |
...
2. **The curvature speed cap is currently too conservative** — the notch has 0.5 m radii, which drives `v_curv ≈ √(a_lat / |κ|)` down toward 1 m/s. On eoh, the forward-clearance-only speed safely cleared the same corners at ~1.3 m/s; the cap is the main reason the advanced corner-mean speed isn't higher. Either raising `lateral_accel_limit` or relaxing the cap to `min(v_eoh, α · v_curv)` with `α > 1` would recover this.
...
<div style="max-width:700px;margin:1.5rem auto 0;">
  ...
  <div style="margin-bottom:14px;">
    <div style="display:flex;justify-content:space-between;font-size:0.92rem;color:#333;margin-bottom:4px;"><span>eoh PID wall-follower in simulation</span><span>100%</span></div>
    <div style="height:7px;background:#e0e0e0;border-radius:3px;overflow:hidden;"><div style="height:100%;width:100%;background:#4a6cf7;border-radius:3px;"></div></div>
  </div>
  <div style="margin-bottom:14px;">
...
**Don't replace a working controller — extend it.** The pure-quadfit first pass was *worse* than eoh on exactly the kinds of corners eoh was tuned for. Keeping eoh as an always-on inner loop and making the fit layer a strictly-bounded perturbation gave us all the upside (smoother straights, corner entry prediction) with a hard guarantee that we never lose the floor.
...
**Arjun Chainani** — *Controls & integration.* eoh port to ROS 2, quadratic fit / health-window design, parameter tuning across tracks.
...
| 3–4  | Jan–Feb 2026 | eoh port + ROS 2 package scaffolding | ✅ Done |
...
# reproduce the eoh-vs-advanced study
